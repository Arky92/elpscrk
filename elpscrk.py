...
def fim():
	...
	global words
	global BOLD
	print OK+BOLD+"[+] Wordlist generated successfully!\nWord generated: {}".format(str(words))
...
def main():
	...
	global BOLD
	printar()
	parser = argparse.ArgumentParser(description="HACKING TOOL FOR WORDLISTS CREATION")
	parser.add_argument('-list', action = 'store', dest = 'lst', help = 'File or word set to create the wordlist')
	...
	if(arg.lst == None):
		...
		if(os.path.isfile(arg.output)):
			print FAIL+BOLD+"[X] Output file already exists!"
			sys.exit(0)
		else:
